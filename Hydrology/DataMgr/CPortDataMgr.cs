...
#pragma warning restore CS0649 // 从未对字段“CPortDataMgr._lastError”赋值，字段将一直保持其默认值 null
        ...
        public void sendBeidouTTCATimer()
        {
            if (addr == "")
            {
                string comPort1 = "COM";
                //端口号
                List<CPortProtocolConfig> m_listPortConfig1 = XmlDocManager.Instance.GetComOrPortConfig(true);
                if (m_listPortConfig1 != null)
                {
                    for (int i = 0; i < m_listPortConfig1.Count; i++)
                    {
                        CPortProtocolConfig cPortProtocolConfig = m_listPortConfig1[i];
                        if (cPortProtocolConfig.ProtocolChannelName == "Beidou-Normal")
                        {
                            comPort1 = comPort1 + cPortProtocolConfig.PortNumber.ToString();
                            break;
                        }
                    }
                }
                ...
                CPortDataMgr.Instance.SendBeidouQSTA(comPort1);
            }
            Thread.Sleep(5000);
            if (addr == "")
            {
            ...
            cTTCAStruct.MsgContent = "1234";
            cTTCAStruct.MsgLength = cTTCAStruct.MsgContent.Length;
            CBeidouNewMgrForm.localCommunicateStr = cTTCAStruct.SenderID;
            ...
            SendBeidouTTCA(comPort, cTTCAStruct);
            CSystemInfoMgr.Instance.AddInfo(string.Format("北斗定时自发自收TTCA命令发送完成"));
        }
        ...
        #region Cable模块数据维护
        private List<ICable> cableLists;
        public void InitCable()
        {
            //  初始化系统中Beidou列表
            if (this.cableLists != null)
            {
                StopCable();
            }
            this.cableLists = new List<ICable>();

            //  配置文件中的北斗卫星（指挥机）名称信息
            var cableNameLists = XmlDocManager.Instance.CableProtocolNames;

            //  系统中存在的串口
            var sysComs = new List<string>(System.IO.Ports.SerialPort.GetPortNames());

            foreach (var cableName in cableNameLists)
            {
                CSystemInfoMgr.Instance.AddInfo(string.Format("配置\"{0}\"......", cableName));
                var dll = XmlDocManager.Instance.GetInfoByName(cableName);
                if (dll == null)
                    continue;
                ...
                foreach (var comNumber in dll.Coms)
                {
                    ...
                    if (!sysComs.Contains(comPortName))
                    {
                        CSystemInfoMgr.Instance.AddInfo(string.Format("配置\"{0}\"......失败!----计算机系统不包含串口{1}!", cableName, comPortName));
                        continue;
                    }
                    //  如果数据库配置中不包含串口配置信息
                    //      获取下一条串口配置信息
                    ...
                    {
                        Debug.WriteLine(string.Format("COM {0} is not the record in database!", comNumber));
                        CSystemInfoMgr.Instance.AddInfo(string.Format("配置\"{0}\"......失败!----数据库中不包含串口{1}信息!", cableName, comPortName));
                        continue;
                    }
                    // 判断配置是否启动
                    if (portInfo.SwitchSatus.HasValue && portInfo.SwitchSatus.Value == false)
                    ...
                    {
                        //  实例化北斗卫星普通终端实例
                        var cable = ProtocolManager.Cable(dll);
                        if (cable == null)
                        {
                            CSystemInfoMgr.Instance.AddInfo(string.Format("配置\"{0}\"......失败!----通讯方式配置出错!", cableName));
                            continue;
                        }
                        //  事件注册
                        //beidou.MessageSendCompleted += CProtocolEventManager.MessageSendCompleted;
                        cable.MessageSendCompleted += CProtocolEventManager.MessageSendCompleted;
                        cable.UpDataReceived += CProtocolEventManager.UpDataReceived;
                        cable.SerialPortStateChanged += CProtocolEventManager.SerialPortStateChanged;
                        //beidou.SoilDataReceived += CProtocolEventManager.SoilDataReceived;
                        ...
                        //  初始化
                        cable.Init(comPortName, portInfo.Baudrate);

                        var dataDllInfo = XmlDocManager.Instance.GetDataDllByComOrPort(comNumber, true);
                        if (dataDllInfo == null)
                        {
                            CSystemInfoMgr.Instance.AddInfo(string.Format("配置\"{0}\"......失败!----数据协议配置出错!", cableName));
                            continue;
                        }
                        var iup = ProtocolManager.Up(dataDllInfo);
                        var idown = ProtocolManager.Down(dataDllInfo);
                        ...
                        var iflash = ProtocolManager.Flash(dataDllInfo);
                        var iSoil = ProtocolManager.Soil(dataDllInfo);
                        cable.InitInterface(iup, idown, iudisk, iflash, iSoil);
                        ...
                        this.cableLists.Add(cable);
                        CSystemInfoMgr.Instance.AddInfo(string.Format("配置\"{0}\"......完成!", cableName));
                    }
                    catch (Exception ex)
                    {
                        // 打开串口失败
        ...
        {
            if (null == this.cableLists)
                return;
            foreach (var cable in cableLists)
            {
                try
                {
                    //  事件解除绑定
                    ...
                    cable.UpDataReceived -= CProtocolEventManager.UpDataReceived;
                    cable.SerialPortStateChanged -= CProtocolEventManager.SerialPortStateChanged;
                    //.SoilDataReceived -= CProtocolEventManager.SoilDataReceived;
                    //beidou.Beidou500BJXXReceived -= CProtocolEventManager.Beidou500BJXXReceived;
                    //beidou.Beidou500SJXXReceived -= CProtocolEventManager.Beidou500SJXXReceived;
                    //beidou.Beidou500ZTXXReceived -= CProtocolEventManager.Beidou500ZTXXReceived;
                    //  关闭北斗卫星
